...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiTool, FiFileText } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'garage-manager', 'mechanic', 'accountant'];
  ...
  const { data, error, isLoading } = useSWR<GarageInterface[]>(
    ...
    () => getGarages({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
